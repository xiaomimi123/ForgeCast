...
export default function TailorPage({ onOpenTailor }: { onOpenTailor: (id: number) => void }) {
  ...
  const inp = 'rounded-md border border-hairline-strong bg-card px-2 py-1 text-sm w-full'
  return (
    <div className="grid grid-cols-[1fr_360px] gap-6">
      <div className="space-y-3">
        {list.data?.map((r) => (
          <div key={r.id} onClick={() => onOpenTailor(r.id)}
            className="card cursor-pointer p-4 hover:border-fire">
        ...
        {list.data?.length === 0 && (
          <div className="rounded-lg border border-dashed border-hairline-strong p-6 text-center text-faint">
            暂无定制需求：右侧录入，或在「分发营销 → 数据复盘」把询单一键转过来
          </div>
        )}
      </div>
      <div className="self-start space-y-2 card p-4">
        ...
        <button className="btn w-full py-1.5 text-sm disabled:opacity-50"
