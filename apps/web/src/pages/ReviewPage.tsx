...
export default function ReviewPage({ onOpenTailor }: { onOpenTailor: (id: number) => void }) {
  ...
  const r = report.data
  const inp = 'rounded-md border border-hairline-strong bg-card px-2 py-1 text-sm'
  return (
    <div className="grid grid-cols-[1fr_360px] gap-6">
      <div className="space-y-4">
        ...
        <div className="card p-4">
          ...
          <ul className="space-y-1 text-sm">
            {leads.data?.map((l) => <li key={l.id} className="border-t border-hairline py-1">[{l.hook ?? '—'}·{l.slug ?? '—'}] {l.wechat ?? '—'} · {l.intent ?? '—'} · {l.status} · {l.created_at}<button className="ml-2 rounded-md border border-hairline-strong px-2 py-0.5 text-xs font-semibold" onClick={() => toTailor(l.id)}>转定制</button></li>)}
      ...
      <div className="space-y-4">
        <div className="card p-4 space-y-2">
          ...
          <button className="btn w-full py-1.5 text-sm" onClick={submitPerf}>提交数据</button>
        </div>
        <div className="card p-4 space-y-2">
          ...
          <button className="btn w-full py-1.5 text-sm" onClick={submitLead}>登记</button>
        </div>
      </div>
    </div>
  )
