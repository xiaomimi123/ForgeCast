 ...
 * （一二三四五六七八九十百千万亿两几），单位/倍数部分沿用原 hyperframes.ts 的 INSIGHT_STAT_RE 字符集。
 ...
 *
 * 单个统一模式（不是两条并列的顶层 `A|B` 分支）：数字部分贪婪匹配后，正则引擎会在结尾单位
 * 匹配失败时自动回溯数字长度，这样才能既让「几万块」测出完整单位，又不破坏「返工率 30%」这种
 * 数字本身就是单位前缀的写法。若拆成顶层 `NUM+倍数 | NUM+范围?+单位` 两条分支，JS 正则按“先
 * 命中哪条就用哪条”而非“取最长”，会在「几万块」上过早命中短分支只吐出「几万」，块字被丢弃
 * ——这正是本轮修复要解决的错误：一张写着错误数字/单位的卡片比没有卡片更误导人。
 *
 * 范围分隔符除阿拉伯数字常见的「-」外，中文口播常说「三到五天」「两至三年」，加入 到/至。
 * 单位候选按长度降序排列——尤其是「个月」必须排在「个」之前，否则「两三个月」会被
 * 「个」提前截断成「两三个」，丢掉月字、意思都变了。
 */
const CN_NUM = '(?:[\\d.]+|[一二三四五六七八九十百千万亿两几]+)'
const RANGE_SEP = '(?:-|到|至)'
// 长度降序：多字单位必须排在其前缀单字单位之前（如 个月 > 个），否则会被短的先截断
const UNIT_ALL = '(?:工作日|个月|小时|分钟|%|％|万|亿|倍|折|天|周|月|年|个|人|元|次|轮|块)'
export const INSIGHT_STAT_RE_CN = new RegExp(`${CN_NUM}(?:${RANGE_SEP}${CN_NUM})?\\s*${UNIT_ALL}`)
