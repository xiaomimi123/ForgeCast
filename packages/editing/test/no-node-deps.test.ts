...
describe('editing 零 Node 依赖', () => {
  // 不用 `new URL('../src', import.meta.url)`——Vite 会静态识别这个字面量写法并把它
  // 当成资源 URL 改写，解析成 http://localhost:3000/src/index.ts 而不是
  ...
  // editing 是纯逻辑包，白名单为空：任何 runtime dependency 都必须先在这里显式过人眼。
  it('package.json dependencies 必须为空', () => {
