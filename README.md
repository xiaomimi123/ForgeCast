...
- `apps/web` Web 控制台（按业务流分五板块：找项目 `/scout` / 拆解需求 `/projects`（读 analysis.md、立项项目按 stage 泳道展示，产物落地自动推进+真实计数，见 docs/superpowers/specs/2026-08-10-projects-board-upgrade-design.md）/ 做内容 `/workshop`（文案+视频生成，含卡点编辑器） / 分发营销 `/market` / 定制项目 `/tailor`）；`templates/` 提示词与封面模板（核心资产）；`workspace/<slug>/` 每项目产物；`workspace/tailor/<id>/` 定制方案书（proposal.md）
